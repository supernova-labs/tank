...
Run (via Bash, always from `/Users/gyprado/dev/tank`):
    ...
    uv run python evals/ablation/query.py --round round2 --run-id L1--none--nointro--r3 --no-introspect "<SurrealQL>"
...
Return the record ids of the passages of the article titled 'Helios processor milestone', in the order they appear in the article (first to last).

## Deliverable

Return your structured output with:
- `run_id`: "L1--none--nointro--r3"
