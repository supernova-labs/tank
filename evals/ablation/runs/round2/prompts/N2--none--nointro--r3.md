...
Tables in the database: article, passage, brief, factbox, topic, covers

## How to query

Run (via Bash, always from `/Users/gyprado/dev/tank`):

    uv run python evals/ablation/query.py --round round2 --run-id N2--none--nointro--r3 --no-introspect "<SurrealQL>"
...
Return the record ids of all DERIVED / machine-generated content units (not original reporting, not curated reference material) whose text mentions 'climate'.

## Deliverable

Return your structured output with:
- `run_id`: "N2--none--nointro--r3"
